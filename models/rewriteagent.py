...
async def rewrite(state: State, config, store):
    """Rewrite an email draft for tone/style using a custom LLM."""
    model = config["configurable"].get("model", "gpt-4o")
    llm = ChatOpenAI(model=model, temperature=0)

    # Grab the last message and its draft content
    prev_message = state["messages"][-1]
    draft = prev_message.tool_calls[0]["args"]["content"]

    # Fetch rewrite instructions from the store, or default to prompt_config["rewrite_preferences"]
    ...
    # Construct the final message to LLM
    ...
    # Construct a new assistant message with the updated draft
    updated_message = {
        "role": "assistant",
        "id": prev_message.id,
