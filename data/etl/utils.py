...
def _database_path() -> str:
    ...
    """
    script_rel_path = os.path.dirname(__file__) if "__file__" in globals() else os.getcwd()
    db_path = os.path.join(script_rel_path, "database.db")  # Store in the same directory as script
...
def upsert_df_to_sql_table(table_name: str, df: pd.DataFrame) -> None:
    ...
    with sqlite3.connect(_database_path()) as con:
        ...
        if not df_to_write.empty:
            df_to_write.to_sql(name=table_name, con=con, if_exists="replace", index=False)
